...
%We should see what happens when we set $c$s to be zero in only one above equations: 
{\bf (proof?)}

The basic idea for counting: Suppose that
\[
F_{ac}+F_{bd}-F_{ad}=0.
\]
Since $F_{ad}$, $F_{bd}$, $F_{ac}\geq 0$, the above equation force $F_{ac}$ and $F_{bd}$ to be zero when $F_{ad}=0$. Therefore, $F_{ad}$ cannot be a true facet of the degeneration. We should count the number of deleted $c$ and such spurious facets. They should be equal. 
