...
We can see what happens from mesh when we set $c$s to be zero in only one above equations: an equation above corresponds a big diamond whose top or bottom corner and all $c$s are zero. For example,
\begin{center}
	\begin{tikzpicture}
	...
	\end{tikzpicture}
\end{center}
this diamond tells us that
\[
A+C-B=0.
\]
Since $A,B,C\geq 0$, the above equation force $A$ and $C$ to vanish when $B=0$. Therefore, $B$ is no longer a facet of the degeneration. Similarly, it's pleasure to show that black points are all vanishing facets in the above example, and generally
