...
(Zhenjie: A possible way to find some $u$-equations of generalized permutohedra:)

For a generic generalized permutohedra $\mathscr P$, it's usually difficult to solve $S_I$ by $\{F_I\}$  from eq.\eqref{GPfacet} and then get $u$-variables, but we can directly use eq.\eqref{GPfacet} to express $x_I$ by $\{u_I\}$ which give us many algebraic equations of $u$-variables. In fact,
\[
...
\]
for any $I\in \mathscr P$, where we introduce a fake $u$-variable $u_{[0,n]}:=x_{[0,n]}$. We know some algebraic equations of $\{x_I\}$, for example, $x_I+x_J=x_{[0,n]}$ for $I\cap J=\varnothing$ and $I\cup J=[0,n]$, so (I will omit all $\text{sth}\in \mathscr P$ for short.)
\[
\prod_{K\supset I}u_K+\prod_{K\supset J}u_K=u_{[0,n]},
\]
or (since we always assume that $u_{[0,n]}=x_{[0,n]}\neq 0$)
\[
\prod_{I\subset K\neq [0,n]}u_K +
\prod_{J\subset K\neq [0,n]}u_K = 1.
\]
(Zhenjie: I guess all possible $u$-equations are consequences of some algebraic equations of $x_I$ which is in fact we have seen in the calculation of $1-u_I$.)
